...
if __name__ == "__main__":
    ...
    job = st.sidebar.selectbox("Choose job to display", job_ids)

    files = [f for f in all_files if f.startswith(f"{job}_")]

    partitions = sorted_names({get_details_from_name(f)[1] for f in files})
    partition_select = st.sidebar.multiselect("Partitions", partitions, partitions)

    files = [f for f in files if get_details_from_name(f)[1] in partition_select]
    ...
    for file in files:
        with open(os.path.join(arg_list.report_dir, file)) as f:
            ...
            round_id, partition = get_details_from_name(file)
